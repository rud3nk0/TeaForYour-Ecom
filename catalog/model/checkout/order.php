...
class ModelCheckoutOrder extends Model {
	public function addOrder($data) {
			...
			$payment_custom      = isset($data['payment_custom_field']) ? json_encode($data['payment_custom_field']) : '';
			$shipping_custom     = isset($data['shipping_custom_field']) ? json_encode($data['shipping_custom_field']) : '';
			...
			$this->db->query("
				INSERT INTO `" . DB_PREFIX . "order` SET
					...
					payment_custom_field     = '" . $this->db->escape($payment_custom) . "',
					...
					shipping_custom_field    = '" . $this->db->escape($shipping_custom) . "',
	...
	public function editOrder($order_id, $data) {
		...
		// 1. Вставка основного заказа
			...
			// 2. Генерация custom_order_id
			$custom_order_id = sprintf("TFY-%04d", $order_id);

			// 3. Обновляем custom_order_id отдельно
			$this->db->query("
			UPDATE `" . DB_PREFIX . "order` 
			SET custom_order_id = '" . $this->db->escape($custom_order_id) . "' 
